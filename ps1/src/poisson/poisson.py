...
def main(lr, train_path, eval_path, save_path):
    ...
    # Prediction plottings
    plt.figure()
    plt.scatter(y_eval, y_pred)
    plt.xlabel('True Count')
    plt.ylabel('Predicted Expected Count')
    plt.title('True vs Predicted Counts (Poisson Regression)')
    plt.savefig(save_path.replace('.txt', 'true_vs_predicted.png'))
    # *** END CODE HERE ***
...
class PoissonRegression:
    ...
    def fit(self, x, y):
        ...
        for i in range(self.max_iter):
            ...
            gradient = x.T @ (y - np.exp(num))

            step = self.step_size * gradient

            theta_new = self.theta + step
